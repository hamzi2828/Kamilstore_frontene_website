...
const categories = [
  { value: "All Categories", labelKey: "header.allCategories" },
...
const priceRanges = [
  ...
  { label: "$100 - $200", labelKey: "catalog.price.100to200", min: 100, max: 200 },
  { label: "Over $200", labelKey: "catalog.price.over200", min: 200, max: Infinity },
];

const discountRanges = [
  { label: "All Discounts", labelKey: "catalog.discount.all", min: 0 },
  { label: "30% or more", labelKey: "catalog.discount.30plus", min: 30 },
  { label: "50% or more", labelKey: "catalog.discount.50plus", min: 50 },
  { label: "70% or more", labelKey: "catalog.discount.70plus", min: 70 },
];
...
function FilterPanel({
  ...
  return (
    <>
      {/* Categories */}
      <div className="mb-6">
        <h4 className="font-semibold text-gray-800 mb-3 text-sm">{t("common.categories")}</h4>
      ...
      {/* Price range */}
      <div className="mb-6">
        <h4 className="font-semibold text-gray-800 mb-3 text-sm">{t("catalog.filters.priceRange")}</h4>
        <ul className="space-y-1">
      ...
      {/* Discount */}
      <div className="mb-6">
        <h4 className="font-semibold text-gray-800 mb-3 text-sm">{t("catalog.filters.discount")}</h4>
        <ul className="space-y-1">
...
export default function FlashSalePage() {
  ...
  const timerUnits = [
    { value: pad(timeLeft.hours), label: t("catalog.flash.hours") },
    { value: pad(timeLeft.minutes), label: t("catalog.flash.min") },
    { value: pad(timeLeft.seconds), label: t("catalog.flash.sec") },
  ];
  ...
  return (
    <>
      <Breadcrumb items={[{ label: t("nav.flashSale") }]} />
              ...
              <div className="ks-fs-header-row">
                  ...
                  <div>
                    <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                      ...
                      <span className="ks-fs-live-badge">
                        ...
                        {t("catalog.flash.liveNow")}
                      </span>
                    </div>
                    <p className="text-xs sm:text-sm text-[#999] font-medium mt-1">
                      {t("catalog.flash.subtitle", { count: filteredProducts.length, total: allProducts.length })}
                    </p>
                  </div>
                </div>
                ...
                <div className="ks-fs-timer-area">
                  <div className="ks-fs-timer-label">
                    ...
                    <span>{t("catalog.flash.saleEndsIn")}</span>
                  </div>
          ...
          <div className="flex gap-6 lg:gap-8">
            ...
            <aside className="hidden lg:block w-64 flex-shrink-0">
              <div className="bg-white rounded-xl p-5 sticky top-[176px]">
                <h3 className="font-bold text-base mb-4 text-[#111]">{t("common.filters")}</h3>
                <FilterPanel
              ...
              <div className="bg-white rounded-2xl p-4 sm:p-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-[12px] sm:text-[13px] text-[#999] font-medium">
                      {t("catalog.showingPrefix")} <strong className="text-[#555]">{filteredProducts.length}</strong> {t("catalog.dealsWord")}
                    </p>
                  </div>
                  ...
                  <div className="ks-fs-sort-wrap">
                    ...
                    >
                      ...
                      <option value="ending">{t("catalog.sort.endingSoon")}</option>
                    </select>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {selectedCategory !== categories[0].value && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-600 rounded-full text-xs font-semibold">
                        {t(categories.find((c) => c.value === selectedCategory)?.labelKey ?? "")}
                        <button onClick={() => setSelectedCategory(categories[0].value)} aria-label={t("catalog.clearCategory")}>
                    ...
                    {selectedPriceRange.label !== priceRanges[0].label && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-600 rounded-full text-xs font-semibold">
                        ...
                        <button onClick={() => setSelectedPriceRange(priceRanges[0])} aria-label={t("catalog.clearPrice")}>
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </span>
                    )}
                    {selectedDiscount.label !== discountRanges[0].label && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-600 rounded-full text-xs font-semibold">
                        {t(selectedDiscount.labelKey)}
                        <button onClick={() => setSelectedDiscount(discountRanges[0])} aria-label={t("catalog.clearDiscount")}>
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </span>
                    )}
                ...
                {filteredProducts.length > 0 ? (
                          ...
                          <div className="ks-fs-sold-section">
                            ...
                            <div className="ks-fs-sold-info">
                              <span className="ks-fs-sold-text">
                                ...
                                {t("catalog.flash.sold", { count: p.sold })}
                              </span>
                              {isHot && (
                                <span className="ks-fs-hot-badge">
                                  <Flame className="w-3 h-3" />
                                  {t("catalog.flash.almostGone")}
                                </span>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-sm font-semibold text-gray-500 mb-1">{t("catalog.flash.noDeals")}</p>
                    <p className="text-xs text-gray-400 mb-4">{t("catalog.flash.tryAdjusting")}</p>
                    <button
                      onClick={() => {
                ...
                <div className="ks-fs-bottom-info">
                  ...
                  <span>{t("catalog.flash.bottomPre")} <strong>{t("catalog.flash.sixHours")}</strong> &middot; {t("catalog.flash.bottomPost")}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-[85%] max-w-sm bg-white overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white">
              <h3 className="font-bold text-base text-[#111]">{t("common.filters")}</h3>
              <button
                onClick={() => setShowFilters(false)}
